<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            勤怠編集（管理者） - {{ $attendance->user->name }}さんの{{ $attendance->work_date->format('Y年m月d日 (D)') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3 class="text-lg font-semibold">勤怠情報編集（管理者権限）</h3>
                        </div>
                        <div class="card-body">
                            <form method="POST" action="{{ route('admin.attendance.update', $attendance->id) }}">
                                ...
                                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                                    <h4 class="font-semibold text-gray-700 mb-2">対象スタッフ</h4>
                                    <p class="text-gray-900">{{ $attendance->user->name }} ({{ $attendance->user->email }})</p>
                                    <p class="text-gray-600 text-sm">勤務日: {{ $attendance->work_date->format('Y年m月d日 (D)') }}</p>
                                </div>
                                ...
                                <div class="mt-6">
                                    ...
                                    <textarea name="notes" class="form-input" rows="3" required placeholder="編集理由を記入してください">{{ old('notes', $attendance->notes) }}</textarea>
