        ...
        <div class="mb-6">
            <div class="flex border-b border-gray-300">
                <a href="{{ route('admin.stamp_correction_request.list', ['status' => 'pending']) }}" 
                   class="px-6 py-3 {{ request('status') === 'pending' || !request('status') ? 'text-gray-800 border-b-2 border-gray-800 font-medium' : 'text-gray-600 hover:text-gray-800 transition-colors' }}">
                    承認待ち
                </a>
                <a href="{{ route('admin.stamp_correction_request.list', ['status' => 'approved']) }}" 
                   class="px-6 py-3 {{ request('status') === 'approved' ? 'text-gray-800 border-b-2 border-gray-800 font-medium' : 'text-gray-600 hover:text-gray-800 transition-colors' }}">
                    承認済み
                </a>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-sm text-gray-900">
                                @if($request->status === 'pending')
                                    <span class="text-orange-600 font-medium">承認待ち</span>
                                @elseif($request->status === 'approved')
                                    <span class="text-green-600 font-medium">承認済み</span>
                                @else
                                    <span class="text-red-600 font-medium">却下</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900">
                                {{ $request->user->name }}
